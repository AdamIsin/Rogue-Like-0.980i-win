...
# Class Room Menu Start <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<        
    menu:
        ...
        "Lock the door" if "lockedclass" not in P_RecentActions:
        ...
        "Unlock the door" if "lockedclass" in P_RecentActions:
                    "You unlock the door"
...
label Shower_Room_Entry:
    ...
    if "showered" not in K_DailyActions and (K_Loc == "bg kitty" or K_Loc == "bg dangerroom"):  #Checks if Kitty is in the shower
            $ Options.append("Kitty")     
    if "showered" not in E_DailyActions and (E_Loc == "bg emma" or E_Loc == "bg dangerroom"):  #Checks if Emma is in the shower
            $ Options.append("Emma")
...
#                        ch_e "I should probably be going. . ."                  #remove this when les stuff is in.                    
#                        call Remove_Girl("Emma")
#                        $ Occupants -= 1
                ...
                if E_Loc == bg_current:
                    if not "classcaught" in E_History:
...
label Study_Room_Explore:
    ...
    if not Line:
            ...
            if D20 >= 10 and "Xavier's photo" not in P_Inventory:            
